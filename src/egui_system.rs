...
use vulkano::{
    buffer::{allocator::{SubbufferAllocator, SubbufferAllocatorCreateInfo}, Buffer, BufferContents, BufferCreateInfo, BufferUsage, Subbuffer}, command_buffer::RenderPassBeginInfo, device::Device, format::{Format, NumericFormat}, image::{sampler::{ComponentMapping, ComponentSwizzle, Filter, SamplerAddressMode, SamplerCreateInfo, SamplerMipmapMode}, ImageAspects, ImageCreateInfo, ImageFormatInfo, ImageSubresourceLayers, ImageType, ImageUsage, SampleCount}, instance::debug::DebugUtilsLabel, memory::{allocator::{AllocationCreateInfo, DeviceLayout, MemoryTypeFilter}, DeviceAlignment}, pipeline::{
        graphics::{
            color_blend::{AttachmentBlend, BlendFactor, ColorBlendAttachmentState, ColorBlendState},
    ...
    }, render_pass::{Framebuffer, RenderPass, Subpass}, swapchain::{Surface, Swapchain}, DeviceSize, NonZeroDeviceSize
};
use vulkano_taskgraph::{command_buffer::{BufferImageCopy, CopyBufferToImageInfo, RecordingCommandBuffer}, descriptor_set::SamplerId, graph::{ExecutableTaskGraph, NodeId, TaskGraph}, resource::{AccessTypes, HostAccessType, ImageLayoutType}, Id, QueueFamilyType, Task, TaskContext, TaskResult};
use vulkano_util::{allocators::Allocators, load_module, resource_access::{GlobalImageCreateInfo, GlobalImageTracker, ModifyImageViewCreateInfo, ResourceAccess}};
...
use crate::{immutable_texture_from_file, utils::ImageCreationError};
type Index = u32;
...
pub trait RenderEguiWorld {
    fn get_framebuffers(&self) -> &Vec<Arc<Framebuffer>>;
    fn get_egui_system<W: 'static + ?Sized>(&self) -> &EguiSystem<W>;
}

/// TODO:
/// * Reimplement automatic render pass creation with
/// correct ImageLayoutType
/// * CallbackFn is not implemented
pub struct EguiSystem<W: 'static + ?Sized> {
    ...
    texture_ids: AHashMap<egui::TextureId, (GlobalImageTracker, SamplerId)>,
...
impl<W: 'static + ?Sized> EguiSystem<W> {
    pub fn new(
        ...
        let font_format = Self::choose_font_format(&access.device());

        let font_sampler = access.create_global_sampler(SamplerCreateInfo {
    ...
    pub fn register_image(
        &mut self,
        image: GlobalImageTracker,
        sampler: SamplerId,
    ) -> egui::TextureId {
        let id = egui::TextureId::User(self.next_native_tex_id);
        self.next_native_tex_id += 1;
        self.texture_ids.insert(id, (image.clone(), sampler));
        id
    }
    ...
    pub fn register_user_image(
        ...
        let sampler = self.access.create_global_sampler(sampler_create_info).unwrap();

        let image = immutable_texture_from_file::<W>(
            &self.access,
            image_file_bytes,
            format,
        )?;

        Ok(self.register_image(image, sampler))
    }
    ...
    fn update_texture_within(
        ...
        let (new_image, image) = if delta.pos.is_some() {
            ...
            let new_image = access.create_global_image::<W>(
                None,
                ImageCreateInfo {
                ...
                GlobalImageCreateInfo::sampled()
                    .modify_image_view_create_info(ModifyImageViewCreateInfo {
                        component_mapping: Some(component_mapping),
                        ..Default::default()
                    }),
            ).unwrap();
            
            (true, (new_image, self.font_sampler))
        };
            ...
            vulkano_taskgraph::execute(
                ...
                |builder, task_context| {
                    ...
                    if new_image {
                        ...
                        // Save!
                        self.texture_ids.insert(id, image);
                    } else {
                ...
                [(image.0.id(), AccessTypes::COPY_TRANSFER_WRITE, ImageLayoutType::Optimal)],
            ).unwrap();
    ...
    pub fn update_textures(
        ...
        let buffer_id = access.resources().create_buffer(
            BufferCreateInfo {
    ...
    /// to [RenderEguiTask].
    pub fn update_task_draw_data<T: 'static + RenderEguiWorld>(
        &mut self,
        task_graph: &mut ExecutableTaskGraph<T>,
        access: &ResourceAccess,
    ) {
        let (clipped_meshes, textures_delta) = self.extract_draw_data_at_frame_end();

        self.update_textures(access, &textures_delta.set);

        for &id in &textures_delta.free {
            self.unregister_image(id);
        }

        let egui_node = task_graph.task_node_mut(self.egui_node_id.unwrap()).unwrap();
        egui_node
            .task_mut()
            .downcast_mut::<RenderEguiTask<T>>()
            .unwrap()
    ...
    fn upload_meshes(
        ...
        let (vertices, indices) = {
            let partition_bytes = total_vertices as u64 * std::mem::size_of::<EpaintVertex>() as u64;
            (
    ...
    pub fn render_egui<T: 'static + RenderEguiWorld>(
        &mut self,
        ...
        let task = RenderEguiTask::new(access, render_pass);
...
pub struct RenderEguiTask<W: ?Sized> {
...
impl<W> RenderEguiTask<W> {
    pub fn new(
        ...
        let pipeline = {
            ...
            let vs = load_module(render_egui_vs::load, access, []);
            let fs = load_module(render_egui_fs::load, access, []);
            ...
            let layout = access.pipeline_layout_from_stages(&stages).unwrap();

            GraphicsPipeline::new(access.device(), None, GraphicsPipelineCreateInfo {
...
impl<W: 'static + RenderEguiWorld + ?Sized> Task for RenderEguiTask<W> {
    type World = W;
            ...
            match primitive {
                Primitive::Mesh(mesh) => {
                    ...
                    if current_texture != Some(mesh.texture_id) {
                        ...
                        builder.as_raw().push_constants(&self.pipeline.layout(), 0, &render_egui_fs::PushConstants {
                            texture_id: texture_id.0.sampled(),
                            sampler_id: texture_id.1,
                            screen_size,
                            output_in_linear_colorspace,
                        })?;
                    };
